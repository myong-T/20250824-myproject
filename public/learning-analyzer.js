// learning-analyzer.js - 학습 결과 분석 시스템 (친근한 AI 선생님 톤 적용)
...
export class LearningAnalyzer {
  ...
  displayDrawingImages(session) {
    try {
      // 마지막 시도의 캔버스 데이터 가져오기
      const lastAttempt = session.attempts[session.attempts.length - 1];
      if (!lastAttempt || !lastAttempt.canvasLines) return;
      
      const canvasData = lastAttempt.canvasLines;
      ...
      Object.keys(canvasData).forEach(key => {
        ...
        if (statsElement && statsElement.parentElement) {
          ...
          // 캔버스 이미지 생성
          const imageData = this.generateCanvasImage(canvasData[key], session.difficulty, key);
      ...
      console.error('도면 이미지 표시 중 오류:', error);
    }
  }
